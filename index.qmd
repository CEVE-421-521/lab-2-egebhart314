...
\begin{align*}
    \text{EAD} &= 0.87\cdot \$0 + 0.1\cdot \$10000 + 0.02\cdot \$50000 + 0.01\cdot \$90000 \\
    &= \$2900
\end{align*}
:::
...
The expected value tends to obfuscate extremes of the distribution, so the *expected* value may be \$2,900 per year, but if a particularly devestating flood occurs, the costs could be much higher
:::
...
let
    ...
    ξ_test = 0.3   # Try: -0.1, 0.0, 0.1, 0.3
...
The peak of the distribution appears wider and is generally flatter as well (taking on a lower maximum value)
:::
...
At low elevations, houses are much more succeptible to most intensities of flooding (including weak, but common floods). However, increasing the elevation just a bit has a major return on investment in protecting against common floods. Since the extreme flooding events are not linearly distributed, it follows that the damage proportion per level of elevation will not be either
:::
...
Since we are using the same storm for every house, the hazard samples are equal so when considering the community vs individual $EAD$, the values are going to be nearly similar. If we used different hazard samples for each house (which may be more physically accurate), the distribution would change.
:::
...
If the probability distribution has uncertainty about the distribution of tail events (particularly if this is a discrete distribution not a theoretrical, continuous one), tail events with probability 0 would not be included in the damage calculation, meaning that the overall figure is underrepresented.
:::
...
Jensen's inequality in general states that $E[\varphi(x)] > \varphi(E[x])$ for any convex function $\varphi(x)$ (in this case, by taking the expected damage at each flood state first we more accurately capture the tails, which $E[x]$ washes out.
:::
...
*What assumptions did we make that might not hold in reality? List at least three and explain why they matter.*
 ...
 - *Elevation is the only parameter of note*: Housing material also matters (e.g. flooding may impact a house made entriely of wood than of concrete)
 - *A single event corresponds to a uniform flood level*: In reality, there is a spatial component to flooding that modulates the hazard for a given house
 - *Constant probability distribution*: The probability distribution is changing in time (this matters when condisering the impacts of climate change)
:::
